...
public class AcademyApplication {
	...
	public static void main(String[] args) {
		...
		try {
			...
			statement.executeQuery("SELECT count(*) FROM pg_database WHERE datname = 'tennisAcademy'");
			ResultSet resultSet = statement.getResultSet();
			resultSet.next();
			int count = resultSet.getInt(1);

			if (count <= 0) {
				statement.executeUpdate("CREATE DATABASE tennisAcademy");
